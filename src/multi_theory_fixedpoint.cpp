...
namespace multi_theory_horn {
    ...
    void MT_fixedpoint::add_predicate_fact(z3::func_decl const& src_decl, z3::expr const& dst_expr, z3::fixedpoint& dst_fp) {
        ...
        std::string fact_name_str = get_fresh_added_fact_name();
        z3::symbol fact_name = m_ctx.str_symbol(fact_name_str.c_str());
        // TODO: Should I create bounds here if dst_expr is int?
        CHC fact(vars, m_ctx.bool_val(true), m_ctx.bool_val(true), dst_new);
        z3::expr added_fact(m_ctx);
        added_fact = fact.get_rule_expr();
    ...
    void MT_fixedpoint::add_interface_constraint(z3::expr p1_expr, z3::expr p2_expr, z3::fixedpoint& fp2) {
        ...
        add_predicate_fact(p1_expr.decl(), p2_expr, fp2);
    }
                        ...
                        for (const z3::expr& bv_body_pred : bv_clause_body_preds) {
                            ...
                            if (int_pred_head_name == bv_body_pred_name) {
                                ...
                                add_interface_constraint(clause_head_pred, bv_body_pred,
                                                         m_mth_fp_set.getBVSolver().fp_solver);
                            }
                        }
                    }
                }
